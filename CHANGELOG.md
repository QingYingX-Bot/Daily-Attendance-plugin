...
- **重大优化**：图片发送方式改进
  - 图片不再存储到磁盘，直接以 base64 格式发送
- **代码优化**：移除不再使用的帮助系统相关代码和路径配置
- **文档更新**：更新 README.md，反映新的图片发送方式

## [1.1.2] - 2025/12/05

- 功能优化：优化过期用户数据恢复逻辑
  - 在签到流程中显式检查用户数据文件是否存在
  - 若用户没有签到数据，自动检查 expired 文件夹并恢复数据
  - 若 expired 中不存在，则使用空数据直接签到（首次签到）
  - 添加日志记录，便于追踪用户数据恢复和首次签到情况
- 代码优化：改进签到流程的数据处理逻辑，使过期用户恢复更加清晰明确
...
- 新增功能：一言统计（`#一言统计` 或 `#yytj` - 查看备用一言库统计信息）
