...
- **Base64 图片发送** - 图片直接以 base64 格式发送，无需存储到磁盘，提升性能
- **自动清理** - 快照数据自动清理，节省磁盘空间
...
- `config/specialDates.json` - 特殊日期配置（可选，用于设置特定日期的运势、一言等）
...
├── data/                  # 数据存储目录
│   ├── user_signs/        # 用户签到数据
│   ├── snapshot/          # 快照数据（每日自动清理）
│   └── expired/           # 过期用户数据（2个月未签到）
├── index.js              # 插件入口文件
├── package.json          # 项目配置文件
└── README.md             # 说明文档
```
